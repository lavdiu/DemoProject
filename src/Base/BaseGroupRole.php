 ...
 * Base Class for Table group_role
 * Basic definition of the fields and relationship with the Database Table group_role
 ...
 */
class BaseGroupRole extends Database\BaseObject
{
	/**
	...
	{
		$this->setTable(new Table('group_role'));
		/**
		 * Generate field data
		 */
		$pk = new PrimaryKey();
		...
		$this->getTable()->addForeignKey(
			(new ForeignKey())
				->setField($this->getTable()->getField("role_id"))
				->setKeyName('group_role_role_id_fk')
				->setReferencingTable("role")
		...
		$this->getTable()->addForeignKey(
			(new ForeignKey())
				...
				->setKeyName('group_role_group_id_fk')
	 ...
	 * @throws InvalidForeignKeyValue
	 */
	public function setRoleIdVal($value = null)
	{
		$this->setFieldValue("role_id", $value);
		return static::returnLeafClass();
	}
	 ...
	 * @return mixed
	 */
	public function getRoleIdVal()
	{
		return $this->getFieldValue("role_id");
	}

	/**
	 * Get Role field reference
	 * @return Field
	 */
	public function getRoleIdFld()
	{
		return $this->getField("role_id");
	}

	/**
	 * Get Role form element reference
	 * @param FormElementInterface|null $formElementOverride
	 * @return ComponentInterface
	 */
	public function getRoleIdFormElement(FormElementInterface $formElementOverride = null) : ComponentInterface
	{
		return $this->getField("role_id")->getFormElement($formElementOverride);
	}

	/**
	 * Get Role Object
	 * @return \Lavdiu\DemoApp\Role
	 */
	public function getRoleIdObj()
	{
		if (is_numeric($this->getRoleIdVal())) {
			return new \Lavdiu\DemoApp\Role($this->getRoleIdVal());
		} else {
			return new \Lavdiu\DemoApp\Role();
		}
	}
